...
template <class CRTP, template <class> class Optional = std::void_t> struct HasFormat : public HasMore<CRTP, HasFormat<CRTP, Optional>> {
    template <std::enable_if_t<!std::is_void_v<Optional<void>>, int> = 0>[[nodiscard]] std::optional<Format> mode() const noexcept {
        return *magic_enum::enum_cast<Format>(this->get_node()->next_attribute("format")->value());
    }

    template <std::enable_if_t<std::is_void_v<Optional<void>>, int> = 0>[[nodiscard]] Format mode() const noexcept {
        return magic_enum::enum_cast<Format>(this->get_node()->next_attribute("format")->value());
    }
};
